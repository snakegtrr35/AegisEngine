...
void CModel::Init(const string* file_name)
{
...
void CModel::Load( const char *FileName )
{
	...
	// 頂点バッファ生成
	{
		...
		CRenderer::GetDevice()->CreateBuffer( &bd, &sd, &pVertexBuffer );
	}
	...
	// インデックスバッファ生成
	{
		...
		CRenderer::GetDevice()->CreateBuffer( &bd, &sd, &pIndexBuffer );
	}

	// サブセット設定
	{
		SubsetArray = new DX11_SUBSET[ model.SubsetNum ];
		SubsetNum = model.SubsetNum;
